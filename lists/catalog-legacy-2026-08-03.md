...
В веб-версии сначала выберите **Инструменты**, **Проекты** или **Гайды**. Каталог проектов
отдельно показывает главный сайт Eclipse Forge и 22 продукта: какую проблему решает каждый,
для кого он сделан, на чём работает и где открыть live-версию или исходный код.

В каталоге находок можно сразу выбрать одну из шести понятных задач — локальный AI,
автоматизацию, исследование, безопасность, media или commerce — либо конкретный проект, а затем открыть карточку:
...
есть отдельные ссылки на Skills, MCP, AI-модели, промпты, security и курсы. Тринадцать
технических типов объединены в семь понятных групп, а дополнительные фильтры по платформе,
лицензии, доверию и проекту раскрываются только когда нужны; на телефоне это компактная нижняя
...
Нужные материалы можно сохранить без аккаунта: избранное и до восьми последних просмотров
остаются только в `localStorage` текущего браузера. Для выбора между похожими вариантами добавьте
две или три карточки к сравнению — Library сопоставит назначение, платформу, стоимость, лицензию,
доверие, риски и применимость к проектам Eclipse Forge.
...
|--------|-------------------------------|--------|
| [GitHub MCP Server](https://github.com/github/github-mcp-server) | Официальный доступ агента к repositories, issues и pull requests. Проверена pinned `0.31.0` stdio-конфигурация: fine-grained token на один repository, `--read-only`, lockdown mode и только нужные toolsets; HTTP mode этой версии не одобрен | [![Stars](https://img.shields.io/github/stars/github/github-mcp-server?style=flat)](https://github.com/github/github-mcp-server) |
...
| [Playwright MCP](https://github.com/microsoft/playwright-mcp) | Официальный Microsoft server для browser automation. Runtime metadata `0.0.78` содержит RCE-equivalent `browser_run_code_unsafe`, а origin allowlist не является security boundary: оставить reference и предпочесть Playwright CLI + Skills для coding workflow | [![Stars](https://img.shields.io/github/stars/microsoft/playwright-mcp?style=flat)](https://github.com/microsoft/playwright-mcp) |
| [Context7 MCP](https://github.com/upstash/context7) | Подтягивает актуальную документацию библиотек в coding-agent. И local package, и remote MCP отправляют query Context7/Upstash; API key даёт authentication/limits, но не локальную приватность — не включать код, secrets и закрытые данные | [![Stars](https://img.shields.io/github/stars/upstash/context7?style=flat)](https://github.com/upstash/context7) |
...
| Проект | Описание |
|--------|----------|
| [AI Code Shield / ЩИТ](guides/ai-code-security-shield.md) | Обязательный pre-release security gate для AI-сгенерированного кода: перед публикацией агент прогоняет текущий репозиторий по security skills, сортирует findings critical→low, исправляет critical/high и объясняет простым языком. База: `Anthropic-Cybersecurity-Skills` + defensive guardrails |
...
| [Eclipse Claw](https://github.com/PavelHopson/Eclipse-Claw) | Rust | Self-hosted web-to-Markdown движок с CLI, REST и MCP. Берёт публичную страницу, убирает лишнюю разметку и возвращает текст/метаданные для research и RAG. По умолчанию блокирует private/metadata сети и опасные redirects, учитывает robots.txt, ограничивает размер/параллелизм и помечает web-текст как недоверенный. LLM и browser вынесены в отдельные authenticated workers; privacy audit не хранит URL/content/secrets. Docker bases закреплены по digest, CI сканирует core/CDP runtime, а release archives защищены SHA256SUMS. External bind, cloud fallback, cookies, proxy DNS и CDP требуют отдельных opt-in; безопасный installer `0.1.4` ещё не опубликован в npm, поэтому используйте [v0.4.2](https://github.com/PavelHopson/Eclipse-Claw/releases/tag/v0.4.2), AGPL-3.0 |
...
| Проект | Для чего нужен | Как используем в Eclipse Forge |
|--------|----------------|--------------------------------|
| [Azgaar’s Fantasy Map Generator](https://azgaar.github.io/Fantasy-Map-Generator/) | Бесплатный браузерный редактор фэнтези-карт: генерирует материки, государства, культуры, биомы, города, реки и дороги; результат можно править и экспортировать в `.map`, SVG, PNG, GeoJSON или JSON | **Внедрить сейчас в Eclipse DnD Forge:** подготовить бриф из текущей кампании, открыть официальный редактор, затем импортировать `Export → JSON → Minimal` с preview, лимитом и защитой от дублей. Карты разрешены для коммерческого использования; подробный безопасный сценарий → [гайд](guides/azgaar-dnd-map-workflow.md) |
...
| [Anthropic Cybersecurity Skills (817)](https://github.com/mukul975/Anthropic-Cybersecurity-Skills) | 817 structured cybersecurity skills для AI-агентов: 29 доменов, MITRE ATT&CK/NIST/ATLAS/D3FEND/AI RMF/F3 mappings, Claude Code/Codex/Cursor/Gemini/20+ платформ. **Defensive use only:** свои проекты, разрешённый аудит, обучение. Практический pre-release workflow → [AI Code Shield / ЩИТ](guides/ai-code-security-shield.md) | [![Stars](https://img.shields.io/github/stars/mukul975/Anthropic-Cybersecurity-Skills?style=flat)](https://github.com/mukul975/Anthropic-Cybersecurity-Skills) |
...
| [AgentShield (часть ECC)](https://github.com/affaan-m/agentshield) | Community security-аудитор для `.claude/`, hooks, MCP servers и tool permissions. Различает активные конфиги, локальные overrides и примеры, проверяет hardcoded secrets, auto-run, prompt injection и unpinned MCP packages. CLI / GitHub Action / GitHub App; findings требуют ручной проверки перед блокировкой CI | [![Stars](https://img.shields.io/github/stars/affaan-m/agentshield?style=flat)](https://github.com/affaan-m/agentshield) |
...
|---|---|---|
| **Agent Reach** | Eclipse Webclaw · Kwork #18 · Eclipse AI Hub · Hopson Sentinel · oh-my-claudecode · Eclipse Media | Исходный skill оставить High-risk reference. В Eclipse-Claw собственным кодом реализованы полезные паттерны: static allowlist и doctor, explicit cloud consent, public-only DNS/redirect policy, untrusted-content boundary, robots/Crawl-delay, отдельные authenticated LLM/CDP workers, privacy-preserving durable audit и fixed regression fixtures ([PR #4](https://github.com/PavelHopson/Eclipse-Claw/pull/4), `51a5d6c`; [PR #5](https://github.com/PavelHopson/Eclipse-Claw/pull/5), `3ef26a8`; [PR #6](https://github.com/PavelHopson/Eclipse-Claw/pull/6), `c5c90a1`; release-патч [PR #7](https://github.com/PavelHopson/Eclipse-Claw/pull/7), `d19a803`; [v0.4.1](https://github.com/PavelHopson/Eclipse-Claw/releases/tag/v0.4.1)). Browser cookies и upstream installer не переносить; YouTube-путь не дублировать поверх transcript-first pipeline; AI Hub/Sentinel/OMC подключать только через отдельный allowlisted adapter с tenant, retention и citation boundaries |
