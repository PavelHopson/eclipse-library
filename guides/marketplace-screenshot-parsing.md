...
>
> **Куда ложится:** Eclipse Market (агрегатор цен — парсинг карточек WB/Ozon/ЯМ),
> Eclipse CRM card-audit (разбор WB-карточки при таймаутах баскетов). ⚠️ Эти репо ведут
...
- **Eclipse Market:** фолбэк-парсер цены/характеристик, когда HTML/анти-бот не отдаёт
  данные — скриншот-путь как «последняя миля». Кэшировать, не дёргать на каждый запрос.
- **Eclipse CRM card-audit:** разбор WB-карточки скриншотом при таймаутах баскетов/CDN
