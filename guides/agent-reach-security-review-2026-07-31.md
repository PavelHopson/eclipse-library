...
|---|---|---|---|---|---|---|---|
| Добавить в roadmap | Eclipse Webclaw / Kwork #18: research-agent не должен молча терять источник | Собственный allowlisted connector registry: capabilities, official API/browser policy, `doctor`, timeout, retry, provenance и controlled fallback | Выше completion rate при понятной причине отказа и цитируемом источнике | L | ToS, SSRF, prompt injection, robots/rate limit, worker isolation | P1 | Описать contract одного public-web connector и измерить success rate/p95 на fixed page set |
