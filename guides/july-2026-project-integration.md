...
- **OmniRoute** - provider-router для AI agents: fallback, cost-aware routing, context compression. ⚠️ Пересекается с уже листнутым **ClawRouter** и `AI_PROVIDER=auto` (Eclipse CRM, фолбэк Groq→Gemini) — не плодить второй роутер; взять как источник доп-идей (context compression, cost-метрики) поверх существующего.
