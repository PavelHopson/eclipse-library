...
- 556 уникальных записей доступны пользователям.
- 112 карточек прошли ручную редакторскую проверку.
- 444 карточки честно помечены как `inferred` и ожидают углублённого review.
- 220 лицензий подтверждаются official GitHub license metadata; ещё 57 условий проверены редакторски.
- 279 лицензий или наборов условий всё ещё требуют отдельной проверки.
- 65 grey-ресурсов видны человеку как reference, но исключены из agent recommendations.
- Всего 72 записи исключены из agent exports по risk, editorial, transport или repository policy.
- 272 записи получили отдельное поле `addedAt`; неизвестные даты не выдумываются.
