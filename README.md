...
- 560 уникальных записей доступны пользователям.
- 116 карточек прошли ручную редакторскую проверку.
- 444 карточки честно помечены как `inferred` и ожидают углублённого review.
- 220 лицензий подтверждаются official GitHub license metadata; ещё 58 условий проверены редакторски.
- 282 лицензии или наборов условий всё ещё требуют отдельной проверки.
...
- 276 записей получили отдельное поле `addedAt`; неизвестные даты не выдумываются.
